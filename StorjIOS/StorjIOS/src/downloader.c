...
static void queue_recover_shards(storj_download_state_t *state)
{
    if (!state->recovering_shards && state->pointers_completed) {
        ...
        if (state->decrypt_key && state->decrypt_ctr) {
            ...
            if (!req->decrypt_key) {
                state->log->warn(state->env->log_options, state->handle, "STORJ MEMMORY ERROR. NO DECRYPTION KEY");
                state->error_status = STORJ_MEMORY_ERROR;
                return;
            }
            req->decrypt_ctr = calloc(AES_BLOCK_SIZE, sizeof(uint8_t));
            if (!req->decrypt_ctr) {
                state->log->warn(state->env->log_options, state->handle, "STORJ MEMMORY ERROR. NO DECRYPTION PONTER");
                state->error_status = STORJ_MEMORY_ERROR;
                return;
            }
